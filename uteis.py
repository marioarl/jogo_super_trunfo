...
def placar(a,b,c,d):
    ...
    print(f"{'':>6}", f"{'_'}"*47)
    print(f"{'JOGADOR:':>15}{len(a)} cartas\033[m    ||||  COMP:    {len(b)} cartas ")
...
def mCartaPre(a):
    print("\n")
    print(f"{Back.YELLOW}{Fore.BLACK}{'*'}"*10,f"{Back.YELLOW}{Fore.BLACK}{'SUA CARTA'}", f"{'*'}"*10 ,f"{'':>2}" , f"{'*'}"*9, f"{'COMPUTADOR'}", f"{'*'}"*9)
